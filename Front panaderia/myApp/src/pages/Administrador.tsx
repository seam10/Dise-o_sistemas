...
import api from '../services/api'; // Asegúrate de importar api.ts directamente
import { Administrador } from '../types/Administrador'; // Importa el tipo Administrador
...
const Login: React.FC = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      // Usamos api.ts directamente para el login
      const admin: Administrador = await api.loginAdministrador(correo, contrasena);
  ...
  const handleWelcomeDismiss = () => {
    ...
    history.push('/producto'); // Redirige a la página deseada después del login exitoso
  };
      ...
      <IonContent className="ion-padding login-content" style={{ backgroundColor: '#f4f4f4' }}>
        ...
        <div style={{ maxWidth: '400px', margin: 'auto' }}>
          <IonItem lines="none" style={{ marginBottom: '20px' }}>
            ...
            <IonLabel position="stacked" style={{ marginLeft: '40px' }}>Correo Electrónico</IonLabel>
            <IonInput
              type="email"
              value={email}
          ...
          <IonItem lines="none" style={{ marginBottom: '30px' }}>
            ...
            <IonLabel position="stacked" style={{ marginLeft: '40px' }}>Contraseña</IonLabel>
            <IonInput
              type="password"
              value={password}
